...
class DiabetesInput(BaseModel):
    ...
    education: float
    income: float
    fruit_veggie_consumption: float
...
class DiabetesEnsembleService:
    ...
    def process_input(self, input_data: DiabetesInput) -> pd.DataFrame:
        return pd.DataFrame([input_data.model_dump()])

    @bentoml.api(route="/predict/ensemble")
    async def predict_ensemble(self, input_data: DiabetesInput) -> dict:
        input_df = self.process_input(input_data)
        # Concurrent predictions via gather
        lgbm_pred, xgb_pred, cb_pred = await asyncio.gather(
